...
inline uint8_t shared_door_status = DOOR_CLOSED_VALUE;
    ...
    while (1)
    {
        ...
        if (xQueueReceive(myQueue, &frame, 0) == pdTRUE)
        {
            if (frame.Type_Frame == UPDATE_FRAME)
            {
                ...
                shared_door_status = frame.Update_values.door_status;
            }
        }
...
void TaskSimulation(void* param)
{
    ...
    while (1)
    {
        ...
        // --- GLOBALNE SPRAWDZANIE BŁĘDÓW STEROWANIA ---
        if (frame.Machine_State != SM_ERROR)
        {
            // [BŁĄD 1] Temperatury / wilgotności poza zakresem
            if (shared_current_temperature < TEMP_MIN_ALLOWED ||
                shared_current_temperature > TEMP_MAX_ALLOWED ||
                shared_current_humidity    < HUMID_MIN_ALLOWED ||
                shared_current_humidity    > HUMID_MAX_ALLOWED)
            {
                Serial.println("[TaskSimulation] ERROR: temp/humid poza zakresem, wygaszam proces");

                // 1. Bezpieczne wygaszenie
                cmd.temp_cmd = TEMP_COOLING;
                cmd.humid_cmd = HUMIDIFY_OFF;
                xQueueSend(cmdQueue, &cmd, portMAX_DELAY);

                // 2. Przejście do stanu błędu
                frame.Machine_State = SM_ERROR;
                state_logged[SM_ERROR] = false;
                state_sent[SM_ERROR] = false;

                vTaskDelay(50 / portTICK_PERIOD_MS);
                continue;   // pomijamy switch, następna iteracja pętli już w SM_ERROR
            }

            // [BŁĄD 2] Otwarcie drzwi podczas aktywnego procesu
            bool active_state =
            ...
            if (active_state && shared_door_status == DOOR_OPEN_VALUE)
            {
                Serial.println("[TaskSimulation] ERROR: drzwi otwarte w trakcie procesu, wygaszam proces");
